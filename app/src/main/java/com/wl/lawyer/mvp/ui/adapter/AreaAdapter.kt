...
class AreaAdapter(data: List<SearchBean.AreaBean>?) :
    BaseQuickAdapter<SearchBean.AreaBean, BaseViewHolder>(
        R.layout.adapter_province,
        data
    ) {
    var mSelectItemPosition = -1

    override fun convert(helper: BaseViewHolder, item: SearchBean.AreaBean?) {
        item?.apply {
