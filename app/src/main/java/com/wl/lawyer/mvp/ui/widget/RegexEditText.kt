...
open class RegexEditText : AppCompatEditText, InputFilter {
    ...
    protected open fun initialize(context: Context, attrs: AttributeSet?) {
        ...
        if (array.hasValue(R.styleable.RegexEditText_inputRegex)) {
            ...
            if (array.hasValue(R.styleable.RegexEditText_regexType)) {
                val regexType = array.getInt(R.styleable.RegexEditText_regexType, 0)
                when (regexType) {
    ...
    companion object {
        ...
        /** 手机号（只能以 1 开头）  */
        val REGEX_MOBILE = "[1]\\d{0,10}"
        /** 中文（普通的中文字符）  */
        val REGEX_CHINESE = "[\\u4e00-\\u9fa5]*"
        /** 英文（大写和小写的英文）  */
        val REGEX_ENGLISH = "[a-zA-Z]*"
        /** 计数（非 0 开头的数字）  */
        val REGEX_COUNT = "[1-9]\\d*"
        /** 用户名（中文、英文、数字）  */
        val REGEX_NAME = "[$REGEX_CHINESE|$REGEX_ENGLISH|\\d*]*"
        /** 非空格的字符（不能输入空格）  */
        val REGEX_NONNULL = "\\S+"
    }
}